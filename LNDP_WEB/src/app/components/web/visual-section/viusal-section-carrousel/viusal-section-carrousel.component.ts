...
export class ViusalSectionCarrouselComponent {
  ...
  scrollImages(offset: number) {
    ...
    if (this.currentIndex < 0) {
      ...
      const imagesPerContainer = Math.floor(
        containerWidth / this.averageImageWidth
      );

      if (
        this.currentIndex >=
        this.carrouselImages.length - imagesPerContainer
      ) {
    ...
    if (this.carouselContainer) {
      ...
      container.style.transform = `translateX(${
        -this.currentIndex * this.averageImageWidth
      }px)`;
    }
  }
}
